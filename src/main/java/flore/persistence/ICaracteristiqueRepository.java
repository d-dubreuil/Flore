...
public interface ICaracteristiqueRepository extends JpaRepository<Caracteristique, Long> {
	...
	@Query("select c from Caracteristique c where c.typeCarac = :type and c.nom = :nom")
	List<Caracteristique> findByTypeEtNom(@Param("type") TypeCarac typeCarac, @Param("nom") String nom);

	@Query("select c from Caracteristique c join c.referentielCaracteristiques rc join rc.flore fl where fl.nom = :nomFlore and c.nom= :nomCarac")
	List<Caracteristique> findByFloreCarac(@Param("nomFlore") String nomFlore, @Param("nomCarac") String nomCarac);

	List<Caracteristique> findByNom(String nom);
	...
	List<Caracteristique> findAllByMalusFl1FL2Carac(@Param("nomFlore1") String nom1, @Param("nomFlore2") String nom2,
			@Param("nomCarac") String nomC);
}
